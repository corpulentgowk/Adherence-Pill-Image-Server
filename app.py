...
def getData():
    dic = imagesRetrieve()
    return jsonify(dic) #Returns images in a JSON Format


#Sample Requests Format
...
#r = requests.post('wmcalyj.pythonanywhere.com/getImages/addModel', files={'test.txt': open('test.txt', 'rb')})
# r = requests.post("http://localhost:5000/addModel", files={'test.bin': open('test.bin', 'rb')})
@app.route('/addModel', methods=['POST']) #Push a Model to Database
def insertModel():
...
#### Uploads File to Parse ####
    ...
    res = connection.getresponse().read() #Retrieves the response of the Post to parse.
    res = str(res) #Converts the response into a string.
...
#### Associates Recently Uploaded File to Parse to Class: Model Storage ####
...
def fetchModel():
    ...
    modelDict = {} #Empty array for models.
